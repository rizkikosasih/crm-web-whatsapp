...
| **Frontend Stack** | AdminLTE 3 + Bootstrap 4 + jQuery      | **TALL Stack** (Tailwind CSS v4 + Alpine.js + Livewire + Laravel) | Desain modern utility-first, penghapusan ketergantungan jQuery, ekosistem yang lebih ringan dan konsisten.                                                                     |
...
### 5.2. TALL Stack — Detail Migrasi Frontend

Migrasi dari AdminLTE 3 (Bootstrap 4 + jQuery) ke **TALL Stack** merupakan perubahan arsitektur frontend terbesar dalam upgrade ini.

#### 5.2.1. Komponen TALL Stack

| Layer                | Teknologi       | Fungsi                                                                                                                                                                                     |
| :------------------- | :-------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **T** — Tailwind CSS | Tailwind CSS v4 | Framework CSS utility-first. Menggantikan AdminLTE 3 dan Bootstrap 4 untuk seluruh styling. Tidak memerlukan `tailwind.config.js` atau PostCSS — konfigurasi langsung via `@theme` di CSS. |
| **A** — Alpine.js    | Alpine.js 3.x   | Library JavaScript ringan untuk interaktivitas sisi klien (dropdown, modal, toggle). Menggantikan jQuery.                                                                                  |
| **L** — Livewire     | Livewire 3.x    | Komponen full-stack reaktif tanpa reload halaman. Sudah digunakan saat ini, tidak ada perubahan arsitektur mayor.                                                                          |
| **L** — Laravel      | Laravel 12      | Framework backend utama.                                                                                                                                                                   |

#### 5.2.2. Dampak Migrasi ke TALL Stack

| Area                    | Perubahan                                                                                                                                                                                                                                                          |
| :---------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **CSS / Styling**       | Seluruh class Bootstrap 4 (`col-md-6`, `btn-primary`, `card`, `table-striped`, dll.) diganti ke utility class Tailwind CSS (`grid grid-cols-2`, `bg-blue-600`, `rounded-lg shadow`, `divide-y`, dll.). File `app.css` dirombak total.                              |
| **JavaScript**          | jQuery (`$()`, `$(document).tooltip(...)`, `$('.error').remove()`) dihapus dan diganti Alpine.js (`x-data`, `x-show`, `x-on:click`, `@click`). Semua dependensi jQuery-based (Select2, Bootstrap JS) diganti alternatif non-jQuery.                                |
| **Komponen UI**         | Komponen AdminLTE (`small-box`, `card-outline`, `sidebar-mini`, `content-wrapper`) di-rebuild dari awal menggunakan Tailwind CSS. Pertimbangkan penggunaan library komponen Blade siap pakai seperti **Flux** atau **WireUI** untuk mempercepat pembangunan ulang. |
| **Chart.js**            | Tetap dipertahankan (tidak terikat jQuery).                                                                                                                                                                                                                        |
...
| **Dependensi Ditambah** | `tailwindcss`, `@tailwindcss/vite`.                                                                                                                                                                                                                                |
